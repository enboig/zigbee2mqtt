...
describe('Homie extension', () => {
    let version;
    let controller;
    let extension;

    let resetExtension = async () => {
        await controller.enableDisableExtension(false, 'Homie');
        MQTT.publish.mockClear();
        await controller.enableDisableExtension(true, 'Homie');
        extension = controller.extensions.find((e) => e.constructor.name === 'Homie');
    }
    ...
    it('Should warn when starting with cache_state false', async () => {
        ...
        await resetExtension();
        expect(logger.warn).toHaveBeenCalledWith("In order for Homie integration to work properly set `cache_state: true");
    });
    ...
    it('Should discover when not discovered yet', async () => {
        controller.extensions.find((e) => e.constructor.name === 'Homie').discovered = {};
        const device = zigbeeHerdsman.devices.WSDCGQ11LM;
        const data = { measuredValue: -85 }
        const payload = { data, cluster: 'msTemperatureMeasurement', device, endpoint: device.getEndpoint(1), type: 'attributeReport', linkquality: 10 };
        MQTT.publish.mockClear();
